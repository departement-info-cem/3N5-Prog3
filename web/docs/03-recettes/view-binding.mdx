...
Nous allons voir comment activer le ViewBinding dans un projet Android.
...
Le Data Binding est plus complexe que le ViewBinding, mais il permet aussi de lier les données de l'application avec l'interface graphique.

Nous vous encourageons lire la comparaison entre les deux méthodes [ici](https://developer.android.com/topic/libraries/view-binding?hl=fr#data-binding).

:::

## Activer le ViewBinding

Pour activer le ViewBinding dans un projet Android, il faut ajouter la ligne suivante dans le fichier `build.gradle` du module `app`:
...
Une fois le ViewBinding activé, vous devez synchroniser le projet.
...
Pour utiliser le ViewBinding, vous devez suivre les étapes suivantes:
...
2. Dans la méthode `onCreate`, initialiser le binding et adaptez le `setContentView` pour utiliser le binding :
...
Cette étape est à refaire dans chaque activité où vous voulez utiliser le ViewBinding.
